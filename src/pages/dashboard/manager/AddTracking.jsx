...
const AddTracking = () => {
  ...
  // 🔹 fetch order info
  useEffect(() => {
    fetch(`https://garmentstracker.vercel.app/api/v1/orders/${id}`)
      .then((res) => res.json())
      .then((data) => {
        setOrder(data);
        setLoading(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        `https://garmentstracker.vercel.app/api/v1/orders/${id}/tracking`,
        {
          method: "PATCH",
    ...
    <div className="px-4 sm:px-6 py-8 sm:py-12">
      <div className="max-w-3xl mx-auto">
        ...
        <form
          ...
          className="bg-white rounded-xl shadow p-4 sm:p-6 space-y-5"
        >
          {/* Status */}
          <div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4">
            <button
              type="submit"
              className="w-full sm:flex-1 py-3 rounded-lg bg-purple-600 text-white font-semibold hover:bg-purple-700 transition"
            >
              Add Tracking
            </button>
            ...
            <button
              ...
              className="w-full sm:flex-1 py-3 rounded-lg border hover:bg-gray-100 transition"
            >
              Cancel
            </button>
          </div>
